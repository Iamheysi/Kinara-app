...
export function ProfileTab({c,t,lang,sessions,restDaysLog,schedule,plans,profileName,setProfileName,profileBio,setProfileBio,profileGoal,setProfileGoal,profilePhoto,setProfilePhoto,photoInputRef,showToast,achievements,onOpenAchievements}){
  ...
  return(<div style={{maxWidth:1100,margin:"0 auto"}}>
    ...
    <div style={{background:c.card,border:`1px solid ${c.border}`,borderRadius:14,padding:"18px 20px"}}>
      ...
      {topEx.length===0?<p style={{fontSize:12,color:c.textMuted}}>{t.noData}</p>:topEx.map(([name,cnt],i)=>(<div key={name} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}><span style={{width:20,display:"inline-flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><KIcon.dumbbell color={i<3?c.primary:c.textMuted} size={16}/></span><div style={{flex:1}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:12.5,color:c.textPrimary}}>{name}</span><span style={{fontSize:11,color:c.textMuted,fontFamily:"'JetBrains Mono',monospace"}}>{cnt}×</span></div><div style={{height:3,background:c.border,borderRadius:2}}><div style={{height:"100%",width:`${(cnt/(topEx[0]?.[1]||1))*100}%`,background:i<3?c.primary:`${c.primary}50`,borderRadius:2}}/></div></div></div>))}
    </div>
  </div>);
}
